...
export async function analyzeAssignment(pdfData) {
	try {
		const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro" });

		// Create the parts array for the content generation
		const parts = [
			{
				text: `Analyze this assignment submission. Provide detailed feedback in the following format:
